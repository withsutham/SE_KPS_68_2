...
export async function GET(
    ...
    try {
        ...
        if (error) {
            console.error("room GET by ID error:", error.message);
            return NextResponse.json({ success: false, error: error.message }, { status: 400 });
        }

        return NextResponse.json({ success: true, data }, { status: 200 });
    } catch (err: any) {
        console.error("room GET by ID exception:", err.message);
...
export async function PUT(
    ...
    try {
        ...
        if (error) {
            console.error("room PUT error:", error.message);
            return NextResponse.json({ success: false, error: error.message }, { status: 400 });
        }

        return NextResponse.json({ success: true, data }, { status: 200 });
    } catch (err: any) {
        console.error("room PUT exception:", err.message);
...
export async function DELETE(
    ...
    try {
        ...
        console.error("room DELETE exception:", err.message);
        return NextResponse.json({ success: false, error: err.message }, { status: 500 });
    }
}
