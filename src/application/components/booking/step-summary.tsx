...
export function StepSummary({ data }: Pick<StepProps, "data">) {
  ...
  const PaymentIcon = data.paymentMethod
    ? PAYMENT_ICON[data.paymentMethod]
    : Banknote;
  ...
  return (
    ...
    >
      ...
      <div className="hidden print:flex flex-col items-center text-center pb-4 border-b border-dashed border-black/30 mb-2 mt-0">
        <h1 className="text-xl font-bold font-mitr text-black mb-1">
          ฟื้นใจ · Massage & Spa
        </h1>
        <p className="text-xs text-black/70 font-sans mb-2">
          123 ถนนเพลินจิต กรุงเทพมหานคร 10110
          <br />
          โทร: 02-123-4567
        </p>
      ...
      <div className="max-w-lg mx-auto w-full bg-card/40 backdrop-blur-sm border border-border/40 rounded-2xl p-6 print:max-w-3xl print:mx-auto print:border-none print:shadow-none print:p-0 print:bg-transparent print:text-black">
        <div className="flex justify-between items-baseline mb-3 print:mb-2">
          <h3 className="font-medium font-mitr text-foreground print:text-black print:text-base">
            รายละเอียดการจอง
          </h3>
          ...
          {data.bookingId && (
            <div className="hidden print:block text-xs font-sans text-black/60">
              เลขที่จอง:{" "}
              <span className="font-mono text-black">{data.bookingId}</span>
            </div>
          )}
        </div>
        <div className="flex flex-col gap-4 print:gap-2 text-sm print:text-xs font-sans print:text-black">
          ...
          <div className="flex flex-col gap-4">
            {data.bookingDetails ? (
              data.bookingDetails.map((detail: any, idx: number) => {
                const service = data.selectedServices.find(
                  (s) => s.massage_id === detail.massage_id,
                );
                return (
                  <div
                    key={idx}
                    className="flex gap-3 items-start print:gap-2 print:border-b print:border-black/10 print:pb-2 print:last:border-0 print:last:pb-0"
                  >
                    <div className="h-8 w-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0 mt-0.5 print:hidden">
                      <Sparkles className="h-4 w-4 text-primary" />
                    </div>
                    <div className="flex-1">
                      <p className="text-xs text-muted-foreground mb-0.5 print:hidden">
                        บริการ
                      </p>
                      <p className="font-medium print:text-sm">
                        {service?.massage_name || "บริการนวด"}
                      </p>
                      <Badge
                        variant="secondary"
                        className="mt-1 text-xs mb-3 print:mb-1 print:border-black/20 print:bg-transparent print:text-[10px] print:px-1.5 print:py-0"
                      >
                        {service?.duration ?? 60} นาที &nbsp;·&nbsp; ฿
                        {Number(service?.massage_price || 0).toLocaleString()}
                      </Badge>
                      ...
                      <div className="grid grid-cols-2 gap-3 bg-secondary/20 p-3 rounded-xl border border-border/40 print:bg-transparent print:border-none print:p-0 print:gap-1">
                        <div>
                          <p className="text-[10px] text-muted-foreground mb-1 uppercase tracking-wider print:text-black/60 print:mb-0 print:text-[8px] print:inline-block print:w-16">
                            พนักงานนวด:
                          </p>
                          <p className="text-sm font-medium text-foreground print:text-black print:text-xs print:inline-block ml-1">
                            {detail.employee_name || "ไม่ได้ระบุ"}
                          </p>
                        </div>
                        <div>
                          <p className="text-[10px] text-muted-foreground mb-1 uppercase tracking-wider print:text-black/60 print:mb-0 print:text-[8px] print:inline-block print:w-8">
                            ห้อง:
                          </p>
                          <p className="text-sm font-medium text-foreground print:text-black print:text-xs print:inline-block ml-1">
                            {detail.room_name || "ไม่ได้ระบุ"}
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <div className="flex gap-3 items-start">
                ...
                <div>
                  ...
                  <Badge variant="secondary" className="mt-1 text-xs">
                    ...
                    นาที &nbsp;·&nbsp; ฿
                    {data.selectedServices
                      .reduce((sum, s) => sum + Number(s.massage_price), 0)
                      .toLocaleString()}
                  </Badge>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 gap-4 print:grid-cols-2 print:gap-y-2 print:gap-x-4 print:py-2 print:border-y border-dashed border-black/30 print:my-0">
            ...
            <div className="flex gap-3 items-center print:gap-0">
              ...
              <div className="print:flex print:items-center print:w-full">
                <p className="text-xs text-muted-foreground mb-0.5 print:text-black/60 print:uppercase print:tracking-wider print:mb-0 print:w-16 print:text-[8px] print:font-semibold">
                  วันที่:
                </p>
            ...
            <div className="flex gap-3 items-center print:col-span-2 print:gap-0">
              ...
              <div className="print:flex print:items-center print:w-full">
                <p className="text-xs text-muted-foreground mb-0.5 print:text-black/60 print:uppercase print:tracking-wider print:mb-0 print:w-[6rem] print:text-[8px] print:font-semibold">
                  วิธีชำระเงิน:
                </p>
                <p className="font-medium print:text-xs print:text-black">
          ...
          <div className="hidden print:block pt-2 mt-1">
            {data.discountAmount && data.discountAmount > 0 ? (
              <div className="flex justify-between items-center text-sm font-medium text-black/70 mb-1">
                <span>ส่วนลด (Discount)</span>
                <span>
                  -฿
                  {data.discountAmount.toLocaleString(undefined, {
            ...
            <p className="text-center text-[10px] text-black/50 mt-4 italic">
              ขอบคุณที่ไว้วางใจใช้บริการฟื้นใจ Massage & Spa
              <br />
      ...
      <div className="flex justify-center gap-4 pt-2 print:hidden">
        <Button
          variant="outline"
          size="lg"
          className="px-6 font-sans gap-2"
