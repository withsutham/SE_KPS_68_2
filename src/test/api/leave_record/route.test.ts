...
import { GET as GET_ALL, POST } from '@/app/api/leave_record/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/leave_record/[id]/route';
...
describe('leave_record API', () => {
  ...
  it('GET /api/leave_record should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/leave_record');
  ...
  it('POST /api/leave_record should create record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/leave_record', 'POST', { name: 'Test' });
  ...
  it('GET /api/leave_record/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/leave_record/123');
  ...
  it('PUT /api/leave_record/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/leave_record/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/leave_record/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/leave_record/123', 'DELETE');
