...
import { GET as GET_ALL, POST } from '@/app/api/booking/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/booking/[id]/route';
...
describe('booking API', () => {
  ...
  it('GET /api/booking should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/booking');
  ...
  it('POST /api/booking should create record', async () => {
  ...
  it('GET /api/booking/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/booking/123');
  ...
  it('PUT /api/booking/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/booking/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/booking/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/booking/123', 'DELETE');
