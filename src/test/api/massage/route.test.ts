...
import { GET as GET_ALL, POST } from '@/app/api/massage/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/massage/[id]/route';
...
describe('massage API', () => {
  ...
  it('GET /api/massage/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/massage/123');
  ...
  it('PUT /api/massage/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/massage/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/massage/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/massage/123', 'DELETE');
