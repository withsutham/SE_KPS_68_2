...
import { GET as GET_ALL, POST } from '@/app/api/coupon/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/coupon/[id]/route';
...
describe('coupon API', () => {
  ...
  it('GET /api/coupon should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/coupon');
  ...
  it('POST /api/coupon should create record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/coupon', 'POST', { name: 'Test' });
  ...
  it('GET /api/coupon/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/coupon/123');
  ...
  it('PUT /api/coupon/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/coupon/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/coupon/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/coupon/123', 'DELETE');
