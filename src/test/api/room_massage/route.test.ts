...
import { GET as GET_ALL, POST } from '@/app/api/room_massage/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/room_massage/[id]/route';
...
describe('room_massage API', () => {
  ...
  it('GET /api/room_massage should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/room_massage');
  ...
  it('POST /api/room_massage should create record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/room_massage', 'POST', { name: 'Test' });
  ...
  it('GET /api/room_massage/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/room_massage/123');
  ...
  it('PUT /api/room_massage/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/room_massage/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/room_massage/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/room_massage/123', 'DELETE');
