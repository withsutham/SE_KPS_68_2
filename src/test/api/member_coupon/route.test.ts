...
import { GET as GET_ALL, POST } from '@/app/api/member_coupon/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/member_coupon/[id]/route';
...
describe('member_coupon API', () => {
  ...
  it('GET /api/member_coupon should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/member_coupon');
  ...
  it('POST /api/member_coupon should create record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/member_coupon', 'POST', { name: 'Test' });
  ...
  it('GET /api/member_coupon/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/member_coupon/123');
  ...
  it('PUT /api/member_coupon/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/member_coupon/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/member_coupon/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/member_coupon/123', 'DELETE');
