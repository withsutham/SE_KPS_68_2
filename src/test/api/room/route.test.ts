...
import { GET as GET_ALL, POST } from '@/app/api/room/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/room/[id]/route';
...
describe('room API', () => {
  ...
  it('GET /api/room should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/room');
  ...
  it('POST /api/room should create record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/room', 'POST', { name: 'Test' });
  ...
  it('GET /api/room/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/room/123');
  ...
  it('PUT /api/room/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/room/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/room/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/room/123', 'DELETE');
