...
import { GET as GET_ALL, POST } from '@/app/api/work_schedule/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/work_schedule/[id]/route';
...
describe('work_schedule API', () => {
  ...
  it('GET /api/work_schedule should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/work_schedule');
  ...
  it('POST /api/work_schedule should create record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/work_schedule', 'POST', { name: 'Test' });
  ...
  it('GET /api/work_schedule/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/work_schedule/123');
  ...
  it('PUT /api/work_schedule/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/work_schedule/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/work_schedule/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/work_schedule/123', 'DELETE');
