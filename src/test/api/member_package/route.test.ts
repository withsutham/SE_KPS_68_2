...
import { GET as GET_ALL, POST } from '@/app/api/member_package/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/member_package/[id]/route';
...
describe('member_package API', () => {
  ...
  it('GET /api/member_package should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/member_package');
  ...
  it('POST /api/member_package should create record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/member_package', 'POST', { name: 'Test' });
  ...
  it('GET /api/member_package/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/member_package/123');
  ...
  it('PUT /api/member_package/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/member_package/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/member_package/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/member_package/123', 'DELETE');
