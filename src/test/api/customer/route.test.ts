...
import { GET as GET_ALL, POST } from '@/app/api/customer/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/customer/[id]/route';
...
describe('customer API', () => {
  ...
  it('GET /api/customer should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/customer');
  ...
  it('POST /api/customer should create record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/customer', 'POST', { name: 'Test' });
  ...
  it('GET /api/customer/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/customer/123');
  ...
  it('PUT /api/customer/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/customer/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/customer/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/customer/123', 'DELETE');
