...
import { GET as GET_ALL, POST } from '@/app/api/payment/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/payment/[id]/route';
...
describe('payment API', () => {
  ...
  it('GET /api/payment should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/payment');
  ...
  it('POST /api/payment should create record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/payment', 'POST', { name: 'Test' });
  ...
  it('GET /api/payment/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/payment/123');
  ...
  it('PUT /api/payment/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/payment/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/payment/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/payment/123', 'DELETE');
