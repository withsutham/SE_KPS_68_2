...
import { GET as GET_ALL, POST } from '@/app/api/package_detail/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/package_detail/[id]/route';
...
describe('package_detail API', () => {
  ...
  it('GET /api/package_detail should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/package_detail');
  ...
  it('POST /api/package_detail should create record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/package_detail', 'POST', { name: 'Test' });
  ...
  it('GET /api/package_detail/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/package_detail/123');
  ...
  it('PUT /api/package_detail/[id] should update record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/package_detail/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/package_detail/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/package_detail/123', 'DELETE');
