...
import { GET as GET_ALL, POST } from '@/app/api/package/route';
import { GET as GET_ONE, PUT, DELETE } from '@/app/api/package/[id]/route';
...
describe('package API', () => {
  ...
  it('GET /api/package should return records', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/package');
  ...
  it('POST /api/package should create record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });

    const req = createMockRequest('http://localhost:3000/api/package', 'POST', { name: 'Test' });
  ...
  it('GET /api/package/[id] should return a single record', async () => {
    mockSupabase.single.mockResolvedValue({ data: { id: '123' }, error: null });
    mockSupabase.auth.admin.getUserById.mockResolvedValue({ data: { user: { id: '123' } }, error: null });

    const req = createMockRequest('http://localhost:3000/api/package/123');
  ...
  it('PUT /api/package/[id] should update record', async () => {
    ...
    const req = createMockRequest('http://localhost:3000/api/package/123', 'PUT', { name: 'Updated' });
  ...
  it('DELETE /api/package/[id] should delete record', async () => {
    mockSupabase.eq.mockResolvedValue({ error: null });

    const req = createMockRequest('http://localhost:3000/api/package/123', 'DELETE');
